package org.openhwgroup.riscfree.globalvariable.view.dsf;
...
import org.openhwgroup.riscfree.globalvariable.view.Activator;
import org.openhwgroup.riscfree.globalvariable.view.datamodel.GlobalVariableDMContext;
import org.openhwgroup.riscfree.globalvariable.view.datamodel.GlobalVariableDMNode;
import org.openhwgroup.riscfree.globalvariable.view.datamodel.IGlobalVariableDMContext;
import org.openhwgroup.riscfree.globalvariable.view.datamodel.IGlobalVariableDescriptor;
...
public class GlobalVariableService extends AbstractDsfService implements IGlobalVariableService {
	...
	public void getGlobalVariableData(IGlobalVariableDMContext variableDmc, DataRequestMonitor<IVariableDMData> rm) {
		...
		IExpressionDMContext expressionDMC = expressions.createExpression(execDmc,getFormattedExpression(getGlobalVariablePath(miVariableDmc), miVariableDmc.getName()));
		...
		expressions.getFormattedExpressionValue(formattedValueContext,
				new DataRequestMonitor<FormattedValueDMData>(getExecutor(), rm) {
					...
					protected void handleSuccess() {
						...
						String name = getFormattedExpression(getGlobalVariablePath(miVariableDmc), miVariableDmc.getName());
					...
					protected void handleFailure() {
						...
						// specific variables!
						rm.done(new VariableData(new MIArg(getFormattedExpression(getGlobalVariablePath(miVariableDmc), miVariableDmc.getName()), Messages.GlobalVariableService_0)));
					}
				});

	}
	
	private String getGlobalVariablePath(GlobalVariableDMContext globalVarCtx) {
		// The relative path could contain backslashes like "../src\\archs-mp.cpp"
		// If we directly use this path for creating variable in GDB it will fail (due to backslashes).
		// So we will rely on the absolute path, if it is present 
		// If Absolute path is not present will convert the double backslashes in relative path to forward slashes and use it
		// Once the absolute path is empty and we are relying on the relative path we have below limitations: 
		// 1. In linux backslash is valid for file name, so if the file name is having double backslashes 
		// it will be replaced with forward slash and it won't work.
		// 2. If the relative path is having more than two backslashes together, 
		// they will be replaces with multiple forward slashes which is invalid.
		// Considering the rarity of these two limitations, we are good for now. 
		// May need to consider handling these also later 
		Supplier<String> processAndFetchRelPath = () -> {
			return Optional.ofNullable(globalVarCtx.getRelativeFilePath()).map(relPath -> {
				if (relPath.contains("\\\\")) {
		...
		return Optional.ofNullable(globalVarCtx.getAbsoluteFilePath()).map((absPath) -> {
			if (absPath.isBlank()) {
				return processAndFetchRelPath.get();
			}
			return absPath;
		}).orElseGet(processAndFetchRelPath);
	}
							...
							if (isSuccess()) {
								...
								infoVariablesResponse.getGlobalVariableList().forEach(global -> {
									initialGlobals.add(new GlobalVariableDMNode(global.getFileName(), global.getFullname(),
											Optional.ofNullable(global.getLine()).map(l -> {
												return Integer.parseInt(l.trim());
											}).orElse(null), global.getName(),
