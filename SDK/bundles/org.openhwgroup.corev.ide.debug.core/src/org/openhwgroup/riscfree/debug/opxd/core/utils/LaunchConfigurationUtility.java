 ...
 * 
 */
package org.openhwgroup.riscfree.debug.opxd.core.utils;
...
import org.openhwgroup.riscfree.debug.opxd.core.Activator;
import org.openhwgroup.riscfree.debug.opxd.core.ConfigurationAttributes;
import org.openhwgroup.riscfree.debug.opxd.core.preferences.DefaultPreferences;
import org.openhwgroup.riscfree.debug.opxd.core.probe.scan.DebugProbe;

import org.openhwgroup.riscfree.debug.opxd.emf.devices.core.OpenHWGroupProcessorDatabaseParser;
import org.openhwgroup.riscfree.debug.opxd.emf.devices.core.OpenHWGroupProcessorDatabaseParser.ARCH_TYPE;
...
public class LaunchConfigurationUtility {
	...
	private static String RISCV_LAUNCH_CONFIG_TYPE = "org.openhwgroup.riscfree.debug.opxd.launchConfigurationType";
	/**
	...
	public static void migrateLaunchConfiguration(ILaunchConfiguration launchConfiguration, MigrationObject migrationObject) {
		try {
			...
			Object transportType = originalAttributes.get("org.openhwgroup.riscfree.debug.opxd.arc.targetDevice");
			
			//if the arc target device keys is null we need to check with ARM launch configuration
			if(transportType==null) {
				transportType = originalAttributes.get("org.openhwgroup.riscfree.debug.opxd.arm.targetDevice");
			}
			if (null == transportType) {
				transportType = originalAttributes.get("org.openhwgroup.riscfree.debug.opxd.probeType");
			}
			//In new configurations the above keys won't be available
			if(transportType != null)
			{
			String tansportTypeValue = (String) transportType;
			originalAttributes.remove("org.openhwgroup.riscfree.debug.opxd.probeType");
			originalAttributes.remove("org.openhwgroup.riscfree.debug.opxd.arc.targetDevice");
			originalAttributes.remove("org.openhwgroup.riscfree.debug.opxd.arm.targetDevice");
	...
	private static void findAndaddCoreNumberAttrToRISCVLaunch(ILaunchConfigurationWorkingCopy workingCopy)
		...
		if (workingCopy.getType().getIdentifier().equals(RISCV_LAUNCH_CONFIG_TYPE)) {
			if (!workingCopy.hasAttribute(ConfigurationAttributes.GDB_SERVER_CORE_NUMBER)) {
				OpenHWGroupProcessorDatabaseParser deviceTreeParser = new OpenHWGroupProcessorDatabaseParser();
