...
import org.openhwgroup.riscfree.debug.opxd.core.dsf.IRISCVCommandFactory;
...
public enum ComRVEngineDebuggingHelper {
	...
	public boolean canExecute(final DsfServicesTracker tracker, DsfSession debugSession, IExecutionDMContext context)
	{
		if(lastDebugSession != debugSession)
		{
			...
			Command comrv_engine_debugging_command = service.getCommand("org.openhwgroup.riscfree.debug.comrv.enableenginedebugging");
			if(comrv_engine_debugging_command != null)
			{
				State state = comrv_engine_debugging_command.getState("org.openhwgroup.riscfree.debug.opxd.overlay.toggleState");
				state.setValue(true);
			}
			service.refreshElements("org.openhwgroup.riscfree.debug.comrv.enableenginedebugging", null);
		}
